...
export default function Login() {
  const [email, setEmail] = useState('demo@finora.ai');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (email === 'demo@finora.ai') {
        try {
          await createUserWithEmailAndPassword(auth, email, password);
          navigate('/dashboard');
          return;
        } catch (regErr: any) {
          if (regErr.code === 'auth/operation-not-allowed' || err.code === 'auth/operation-not-allowed') {
            setError('Please enable "Email/Password" sign-in provider in your Firebase Console -> Authentication.');
          } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-dark-900 px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2">Login to manage your finances smartly.</p>
        </div>
